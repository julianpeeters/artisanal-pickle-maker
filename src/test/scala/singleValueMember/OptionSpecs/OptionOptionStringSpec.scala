...
class OptionOptionStringSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_OptionOptionString"), List(("oa", "Option[Option[String]]")))
  ...
  "a ScalaSig for case class MyRecord_OptionOptionString(oa: Option[Option[String]])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_OptionOptionString]).map(ScalaSigAttributeParsers.parse(_)).get
