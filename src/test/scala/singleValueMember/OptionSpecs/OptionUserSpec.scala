...
class OptionUserSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_OptionUser"), List(("oa", "Option[MyRecord_User]")))
  ...
  "a ScalaSig for case class MyRecord_OptionUser(oa: Option[MyRecord_User])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_OptionUser]).map(ScalaSigAttributeParsers.parse(_)).get
