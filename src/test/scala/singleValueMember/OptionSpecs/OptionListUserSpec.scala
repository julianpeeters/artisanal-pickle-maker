...
class OptionListUserSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_OptionListUser"), List(("oa", "Option[List[MyRecord_User]]")))
  ...
  "a ScalaSig for case class MyRecord_OptionListUser(oa: Option[List[MyRecord_User]])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_OptionListUser]).map(ScalaSigAttributeParsers.parse(_)).get
