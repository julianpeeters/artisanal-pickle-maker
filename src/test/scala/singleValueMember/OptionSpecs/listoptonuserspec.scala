...
class ListOptionUser_Spec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_ListOptionUser_"), List(("oa", "List[Option[MyRecord_User]]")))
  ...
  "a ScalaSig for case class MyRecord_ListOptionUser_(oa: List[Option[MyRecord_User]])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_ListOptionUser_]).map(ScalaSigAttributeParsers.parse(_)).get
