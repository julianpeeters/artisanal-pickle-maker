...
class ListFloatSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_ListFloat"), List(("t", "List[Float]")))
  ...
  "a ScalaSig for case class MyRecord_ListFloat(t: List[Float])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_ListFloat]).map(ScalaSigAttributeParsers.parse(_)).get
