...
class ListUser2Spec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_ListUser2"), List(("af", "List[MyRecord_User]")))
  ...
  "a ScalaSig for case class MyRecord_ListUser2(af: List[MyRecord_User])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_ListUser2]).map(ScalaSigAttributeParsers.parse(_)).get
