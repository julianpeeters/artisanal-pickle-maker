...
class ListAnySpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_ListAny"), List(("ab", "List[Any]")))
  ...
  "a ScalaSig for case class MyRecord_ListAny(ab: List[Any])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_ListAny]).map(ScalaSigAttributeParsers.parse(_)).get
