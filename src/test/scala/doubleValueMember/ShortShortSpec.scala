...
class ShortShortSpec extends mutable.Specification {
  ...
  "a ScalaSig for case class MyRecord_ShortShort(b1: Short, b2: Short)" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_ShortShort]).map(ScalaSigAttributeParsers.parse(_)).get
    val myParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_ShortShort]).map(ScalaSigAttributeParsers.parse(_)).get
