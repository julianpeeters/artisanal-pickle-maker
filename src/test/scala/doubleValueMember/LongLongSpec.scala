...
class LongSpecLong extends mutable.Specification {
  ...
  "a ScalaSig for case class MyRecord_LongLong(d1: Long, d2: Long)" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_LongLong]).map(ScalaSigAttributeParsers.parse(_)).get
    val myParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_LongLong]).map(ScalaSigAttributeParsers.parse(_)).get
