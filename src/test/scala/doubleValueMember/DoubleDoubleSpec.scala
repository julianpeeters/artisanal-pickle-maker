...
class DoubleDoubleSpec extends mutable.Specification {
  ...
  "a ScalaSig for case class MyRecord_DoubleDouble(f1: Double, f2: Double)" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_DoubleDouble]).map(ScalaSigAttributeParsers.parse(_)).get
    val myParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_DoubleDouble]).map(ScalaSigAttributeParsers.parse(_)).get
