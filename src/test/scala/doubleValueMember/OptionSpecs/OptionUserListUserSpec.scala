...
class OptionUserListUserSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_OptionUserListUser"), List(("oc1", "Option[MyRecord_User]"), ("oc2", "List[MyRecord_User]")))
  ...
  "a ScalaSig for case class MyRecord_OptionUserListUser(oc1: Option[MyRecord_User], oc2: List[MyRecord_User])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_OptionUserListUser]).map(ScalaSigAttributeParsers.parse(_)).get
