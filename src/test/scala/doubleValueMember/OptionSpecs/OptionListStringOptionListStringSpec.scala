...
class OptionListStringOptionListStringSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_OptionListStringOptionList"), List(("oa1", "Option[List[String]]"), ("oa2", "Option[List[String]]")))
  ...
  "a ScalaSig for case class MyRecord_OptionListStringOptionList(oa1: Option[List[String]], oa2: Option[List[String]])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_OptionListStringOptionList]).map(ScalaSigAttributeParsers.parse(_)).get
