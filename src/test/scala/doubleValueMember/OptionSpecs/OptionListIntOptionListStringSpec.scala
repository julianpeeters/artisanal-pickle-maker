...
class OptionListIntListOptionStringSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_OptionListIntListOptionString"), List(("oa1", "Option[List[Int]]"), ("oa2", "List[Option[String]]")))
  ...
  "a ScalaSig for case class MyRecord_OptionListIntListOptionString(oa1: List[Option[Int]], oa2: List[Option[String]])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_OptionListIntListOptionString]).map(ScalaSigAttributeParsers.parse(_)).get
