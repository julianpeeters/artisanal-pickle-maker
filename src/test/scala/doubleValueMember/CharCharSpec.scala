...
class CharCharSpec extends mutable.Specification {
  ...
  "a ScalaSig for case class MyRecord_CharChar(g1: Char, g2: Char)" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_CharChar]).map(ScalaSigAttributeParsers.parse(_)).get
    val myParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_CharChar]).map(ScalaSigAttributeParsers.parse(_)).get
