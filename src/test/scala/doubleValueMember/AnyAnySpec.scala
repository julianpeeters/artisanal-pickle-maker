...
class AnyAnySpec extends mutable.Specification {
  ...
  "a ScalaSig for case class MyRecord_AnyAny(m1: Any, m2: Any)" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_AnyAny]).map(ScalaSigAttributeParsers.parse(_)).get
    val myParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_AnyAny]).map(ScalaSigAttributeParsers.parse(_)).get
