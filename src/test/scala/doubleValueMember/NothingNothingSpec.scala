...
class NothingNothingSpec extends mutable.Specification {
  ...
  "a ScalaSig for case class MyRecord_NothingNothing(l1: Nothing, l2: Nothing)" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_NothingNothing]).map(ScalaSigAttributeParsers.parse(_)).get
    val myParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_NothingNothing]).map(ScalaSigAttributeParsers.parse(_)).get
