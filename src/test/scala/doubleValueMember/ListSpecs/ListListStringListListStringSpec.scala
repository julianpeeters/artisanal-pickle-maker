...
class ListListStringListListStringSpec extends mutable.Specification {
  ...
  val mySig = new artisinal.pickle.maker.ScalaSig(List("case class"), List("models", "MyRecord_ListListStringListListString"), List(("r1", "List[List[String]]"), ("r2", "List[List[String]]")))
  ...
  "a ScalaSig for case class MyRecord_ListListStringListListString(r1: List[List[String]], r2: List[List[String]])" should {
    ...
    val correctParsedSig = parseByteCodeFromAnnotation(classOf[MyRecord_ListListStringListListString]).map(ScalaSigAttributeParsers.parse(_)).get
