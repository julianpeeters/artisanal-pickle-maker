...
import tags._
import types._
class TypeRefTpes(position: Position, stores: Stores, classSym: ClassSym, thisTpes: ThisTpes, extModClassRefs: ExtModClassRefs) {
...
//basic scala datatypes
  ...
  val anyRef = TypeRefTpe_AnyRef(position, stores, thisTpes.scala, extModClassRefs.scala)

//more datatypes
  val obj = TypeRefTpe_Object(position, stores, thisTpes.scala, thisTpes.lang, extModClassRefs.lang, extModClassRefs.java)
  ...
  val uncheckedVariance = TypeRefTpe_UncheckedVariance(position, stores, extModClassRefs.unchecked, extModClassRefs.annotation, extModClassRefs.scala)
...
//generics
  ...
  def option(boxedType: Tpe) = TypeRefTpe_Option(position, stores, thisTpes.scala, extModClassRefs.scala, boxedType)

  def stream(boxedType: Tpe) = TypeRefTpe_Stream(position, extModClassRefs.scala)
  val iterator = TypeRefTpe_Iterator(position, stores, thisTpes.collection, any, extModClassRefs.collection, extModClassRefs.scala)

  def listNoBoxed            =  TypeRefTpe_ListNoBoxed(position)//to keep track of the first list written, indepenent of the boxed type
  def list(valueMemberName: String, boxedType: Tpe) = TypeRefTpe_List(position, stores, noneSym, extModClassRefs.scala, thisTpes.scala, extModClassRefs.predef, valueMemberName, boxedType)

//user-defined classes
  def userDefined(typeName: String, ownerName: String) = TypeRefTpe_userDefined(position, stores, typeName, ownerName, thisTpes.owner, extModClassRefs )
