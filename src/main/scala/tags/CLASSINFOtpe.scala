...
case class CaseClassInfoTpe(currentPosition: Position, classSym: ClassSym, AnyRef: TypeRefTpe_AnyRef, Product: TypeRefTpe_Product, Serializable: TypeRefTpe_Serializable) {
  var position = 0
  ...
  //data {
    ...
    //reference to entry 12, Product typereftype 
      Product.position match {
        case 0      => myPickleBuffer.writeNat(currentPosition.current + 7)//TYPEREFs for types not already defined need to be added next
        case i: Int => myPickleBuffer.writeNat(i)
      }
    //reference to entry 15, Serializable typereftype
      Serializable.position match {
        case 0      => myPickleBuffer.writeNat(currentPosition.current + 10)//TYPEREFs for types not already defined need to be added next
        case i: Int => myPickleBuffer.writeNat(i)
      }
...
case class ClassInfoTpe(currentPosition: Position, classSym: ClassSym, AnyRef: TypeRefTpe_AnyRef) {
  var position = 0
  def write(myPickleBuffer: PickleBuffer)  = {
    position = currentPosition.current
  //tag
